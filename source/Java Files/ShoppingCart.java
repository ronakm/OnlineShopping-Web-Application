...
public class ShoppingCart {
  ...
  public synchronized void addItem(String itemID,String filePath) {
    ItemOrder order;
    ...
    ItemOrder newOrder = new ItemOrder(getItem(itemID,filePath));
   ...
   *  item from cart.
   */
  public static CatalogItem getItem(String itemID,String filePath)
  {
	CatalogItem e=null;
	try
		{
		 ...
		 HashMap catmap = (HashMap)srpin.readObject();
		 e= (CatalogItem)catmap.get(itemID);
  ...
  public synchronized void setNumOrdered(String itemID,int numOrdered,String filePath) {
    ItemOrder order;
    for(int i=0; i<itemsOrdered.size(); i++) {
      order = (ItemOrder)itemsOrdered.get(i);
      if (order.getItemID().equals(itemID)) {
    ...
    ItemOrder newOrder =
      new ItemOrder(getItem(itemID,filePath));
    itemsOrdered.add(newOrder);
  }
}
